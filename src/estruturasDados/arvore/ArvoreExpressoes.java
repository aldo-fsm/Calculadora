...
public class ArvoreExpressoes {
	...
	private BigDecimal auxCalcularExpressao(Node<String> raiz, ListaArray<String> variaveis,
		...
		if (root != null) {
			...
			case "*":
				return auxCalcularExpressao(raiz.getLeftNode(), variaveis, valoresCorrespondentes)
						.multiply(auxCalcularExpressao(raiz.getRightNode(), variaveis, valoresCorrespondentes));
			case "/":
				return auxCalcularExpressao(raiz.getLeftNode(), variaveis, valoresCorrespondentes).divide(
						auxCalcularExpressao(raiz.getRightNode(), variaveis, valoresCorrespondentes), mathContext);
			case "-":
				return auxCalcularExpressao(raiz.getLeftNode(), variaveis, valoresCorrespondentes)
						.subtract(auxCalcularExpressao(raiz.getRightNode(), variaveis, valoresCorrespondentes));
			case "+":
				return auxCalcularExpressao(raiz.getLeftNode(), variaveis, valoresCorrespondentes)
						.add(auxCalcularExpressao(raiz.getRightNode(), variaveis, valoresCorrespondentes));
			case "^":
				BigDecimal retorno = auxCalcularExpressao(raiz.getLeftNode(), variaveis, valoresCorrespondentes)
						.pow(auxCalcularExpressao(raiz.getRightNode(), variaveis, valoresCorrespondentes).intValue());
				BigDecimal numerosAposVirgula = auxCalcularExpressao(raiz.getRightNode(), variaveis,
						valoresCorrespondentes)
								.subtract(BigDecimal.valueOf(
										auxCalcularExpressao(raiz.getRightNode(), variaveis, valoresCorrespondentes)
												.intValue()));
				int valorRaiz = 1;
	...
	private BigDecimal raiz(BigDecimal valor, int indice, int iteracoes) {
		...
		for (int i = 0; i < iteracoes; i++) {
			resultado = resultado.subtract(resultado.pow(indice).subtract(valor)
					.divide(resultado.pow(indice - 1).multiply(BigDecimal.valueOf(indice)), mathContext));
		}
		return resultado;
	}
}
